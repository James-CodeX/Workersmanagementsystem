...
export default function AccountRow({ account, employees }: { account: Account; employees: Employee[] }) {
    ...
    const [editData, setEditData] = useState({
        ...
        accountType: account.accountType || "Outlier",
        notes: account.notes || "",
    });
    ...
    async function handleEdit(e: React.FormEvent<HTMLFormElement>) {
        ...
        formData.append("accountType", editData.accountType);
        ...
        <>
            <tr>
                <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-white">
                    {account.accountName}
                    {account.notes && (
                        <div className="text-xs text-gray-400 mt-1 max-w-xs truncate" title={account.notes}>
                            {account.notes}
                        </div>
                    )}
                </td>
